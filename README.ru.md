...
геттер типа `LogFn`, который принимает либо функцию, либо `closure`
...
разница между ними — около 1 нс. AOT: 10.25 нс замыканию против 11.12 нс
методу при sd 0.05 и 0.07 — разница настоящая и ничтожная. JIT: 11.75
против 11.99 при sd 0.25 и 0.28 — разницы нет вовсе. Берите тот, который
лучше читается.
...
- **`maxQueueSize`** — сколько записей очередь принимает, прежде чем
  ...
  сейчас. Умолчание — 100 000, это около 20 МБ на логах по двести байт.
  На пределе отказ получает **входящий** лог: он уходит в `onDropped`
  и в очередь не попадает, поэтому всё уже принятое всё равно будет
  доставлено, а `flush()` и `close()` значат ровно то же, что и раньше.
  Очередь разбирается только на обороте цикла событий, поэтому плотный
  цикл, публикующий больше границы и ничего не ожидающий, теряет остаток
  при сколь угодно живом стоке; именно на такой цикл, а не на вставший
  сток, и рассчитано умолчание — собственный бенчмарк пакета публикует
  20 000 логов ровно такой пачкой, и границы хватает на неё целиком.
  `null` — сознательный отказ от границы: очередь тогда растёт, пока
...
- **`onDropped`** — вызывается с тем, что отброшено. У всех восьми это
  ...
  сам, напечатав первую сразу и сосчитав остальные в сводку — её напечатает
  следующая потеря, пришедшая больше чем через пять секунд (с расширением
  до минуты, пока потери идут), или `close()`. Таймера за этим нет
  намеренно, и следствие стоит знать: пачка, кончившаяся без новой потери
  и без `close()`, останется с одной первой строкой и без счёта. Говорит
  он через `print`, то есть в stdout приложения, — стоит подумать, если
  ваш stdout несёт протокол, а не консоль. `onDropped: (_) {}` заглушает
  всё это, а свой `print` перенаправляет: `print` идёт через текущую
  зону. Небуферизованные четыре отдают по
...
Очередь перед файлом по умолчанию держит 100 000 записей: если диск
...
конструктора. `registerLevels` — всегда: оба конструктора зовут его из
базового класса. `processLog` — всякий раз, когда саблогер наследует
уровень, уже включённый у родителя: `CustomLogger.sub` регистрирует
уровни, а затем берёт уровень родителя, так что уровень включается,
пока тело ещё не выполнялось.

Инициализаторы полей и список инициализации к этому моменту *уже*
отработали — потому уровни в `final`-полях и работают. А всё, что
присваивает тело, ещё нет, и `late`-поле, прочитанное там, бросает
исключение. Держите оба хука подальше от того, что трогает тело, или
перенесите эту работу в инициализатор поля.
