...
читается. Этот фрагмент предполагает `Logger` из раздела
[Иерархические логгеры](#иерархические-логгеры) — там `child` и
определён.
...
дал 10.4 нс замыканию против 11.9 нс методу, JIT — 11.9 против 11.7.
Берите тот, который лучше читается.
...
включённом уровне замыкание добавляет около 3 нс к вызову, который и так
стоит ~135 нс, а при выключенном превращает 41 нс в 4 нс. Погрешность,
когда вы проигрываете, и порядок величины, когда выигрываете.

Тер-офф уже существующей функции дешевле ещё: `log.d(buildMessage)` не
аллоцирует ничего на вызов и даёт 1.9 нс на выключенном уровне против
3.7 нс у `log.d(() => ...)`, который создаёт по замыканию на каждый
вызов — независимо от того, включён уровень или нет. Оба варианта далеко
ниже 41 нс жадной сборки строки. (Замерено на 1 млн вызовов каждого
варианта, AOT; у JIT числа другие по масштабу, но не по картине.)
...
Каждый из восьми принимает одни и те же три необязательных аргумента:
...
- **`retryDelay`** — только у буферизованных вариантов: сколько ждать
  перед повторной попыткой для пачки, возвращённой через `retryBuffer`.
  Умолчание `Duration.zero` всё равно проходит через цикл событий,
  поэтому мёртвый приёмник не может заморить голодом таймеры или ваш же
  `close()`, но повтор идёт так быстро, как позволяет цикл. Задайте
  настоящую задержку, если приёмник может быть недоступен какое-то
  время, — и учтите, что `close()` тогда дождётся отложенного повтора.
...
> [!IMPORTANT]
> Все эти очереди **неограниченны**. Если приёмник не справляется,
> ожидающие логи копятся, пока процессу не кончится память. Ни политики
> переполнения, ни счётчика отброшенных логов нет — ограничивайте вход
> сами, если приёмник может встать. В буферизованных вариантах
> `retryBuffer` — единственное, что сохраняет лог при сбое: бросивший
> обработчик теряет всё, что не вернул в буфер.
...
**Что наследуется.** Три вещи, у каждой своя связь: `level`,
поуровневые паблишеры и `transformer`. Изменение у родителя доходит до
каждого саблогера, у которого соответствующая связь ещё цела:

```dart
root.level = Levels.debug; // db и http тоже переключаются на debug
```

**Как саблогер отвязывается.** Присваивание любой из трёх вещей прямо
на саблогере рвёт эту связь — с этого момента саблогер держит своё
значение и родителя игнорирует:
...
только для корневого логгера:

```dart
http.relink(); // снова следует за root
```
...
У `TransformPublisher` есть собственный `onError`; без него ошибки
уходят в текущую зону. `flush` и `close` делегируются обёрнутому
паблишеру.
...
> трансформера.
>
> Страховка синхронная и своя у каждого логгера. Она не ловит цикл,
> проходящий через **саблогер**, унаследовавший тот же трансформер
> (саблогер — отдельный логгер со своей страховкой), и не переживает
> **асинхронного скачка**: трансформер, откладывающий вложенный лог
> через `scheduleMicrotask` или `Future`, оказывается вне страховки
> целиком, и безусловный вариант будет крутиться вечно.
...
Очередь перед файлом неограниченна: если диск встанет, ожидающие пачки
будут копиться в памяти, пока процесс не умрёт. Это плата за то, что ни
один лог не теряется молча, — см.
[полный набор](#полный-набор) про `retryDelay` и остальное.
