...
- **`maxQueueSize`** — сколько записей очередь принимает, прежде чем
  ...
  ровно то же, что и раньше. `null` — сознательный отказ от границы:
...
- **`onDropped`** — вызывается с тем, что отброшено. У всех восьми это
  ...
  Без этого колбэка потеря не оставляет следа. Небуферизованные четыре
  отдают по одному логу за раз (и `param` вместе с ним, где он есть),
...
> отказ и уходит в `onDropped` — задайте его, иначе давление будет
> стоить вам логов без следа. Ничего уже принятого не теряется, поэтому
