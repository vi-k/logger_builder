...
Все восемь принимают одни и те же три необязательных аргумента:
...
- **`retryDelay`** — только у буферизованных вариантов: сколько ждать
  ...
  время; она удваивается с каждой попыткой, с потолком в 32 базовых.
  `close()` ничего из этого не выжидает — он гасит взведённый таймер
...
- **`onDropped`** — вызывается с тем, что отброшено, а происходит это
  двумя путями: пачка исчерпала бюджет `maxRetries`, и записи, возвращённые
  в `retryBuffer` уже после вызова `close()`, — обработать их некогда.
  Без этого колбэка и то и другое пропадает без следа.
...
> ожидающие логи копятся, пока процессу не кончится память. Политики
> переполнения нет, и никто не считает, во что оно вам обошлось, —
> ограничивайте вход сами, если приёмник может встать. (`onDropped` — не
> этот счётчик: он сообщает о бюджете повторов и о закрытии, не
> о давлении.)
>
