...
final class Logger
  ...
  Logger(String name)
    : // - `UnmodifiableListView` is used to prevent path modification.
      //   Accordingly, when passing the path to `LogEntry`, there is no need
      //   to make a copy of the list: this saves both speed and memory.
      // - A list of one element is created using `List.filled`, as this is
      //   faster and more economical than the `[name]`.
      // - `UnmodifiableListView` is used instead of `List.unmodifiable` to
      //   avoid making an extra copy of the list: this saves both speed and
      //   memory. However, this is only possible when we know for sure that
      //   the original list will not change.
      path = UnmodifiableListView(List.filled(1, name));

  Logger._(super.parent, String name)
    : // A new list (a copy of the parent's list plus the current logger's
      // name) is created using `List.generate` with `growable: false`,
      // as this is faster and more economical than the `[name]`.
