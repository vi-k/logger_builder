> **Состояние на 2026-08-16:** все находки закрыты — B1–B5, B9 и часть
> P1 в релизе 0.3.3 (`8ad22b4`), остальное в 0.4.0 (`e05af04`);
> исключения зафиксированы в плане: M3 (5.6) не исполнялся по решению Р6,
> D1/T2 доведены до конца в 0.4.0. Документ исторический, состояние
> кодовой базы в нём — на 0.3.2.
> **Что это:** полное ревью кодовой базы v0.3.2 с находками B/M/D/T
> и шкалой серьёзности; исходное основание для плана доработок.
> **Связанные записи:** `2026-07-29[2]-0.3.3-0.4.0-plan.md`,
...
- **Связанный документ:** [техзадание на доработки](2026-07-29%5B2%5D-0.3.3-0.4.0-plan.md)
  по итогам этого ревью. Нумерация находок (B/M/D/T) единая в обоих документах.
...
библиотека использовать его не будет (см. план, п. 4.17, опционально 5.6).*
...
- `TODO.md` на русском при остальной документации на английском. *Решение автора:
  остаётся на русском, исключён из публикации через `.pubignore` (см. план, п. 4.14).*
...
## 7. Кросс-ревью реализаций

Кросс-ревью реализации milestone 1 (v0.3.3, находки CR1–CR4, F4, F5) —
в записи `2026-07-29[3]-0.3.3-cross-review-report.md`.

Кросс-ревью реализации milestone 2 (v0.4.0, находки CR5–CR9) —
в записи `2026-07-29[4]-0.4.0-cross-review-report.md`.

## 8. Соответствие находок пунктам плана

| ID | Пункт плана | ID | Пункт плана |
|---|---|---|---|
| B1 | 4.1 | M2 | 4.10 |
| B5 | 4.2 | M3 | 4.17 → 5.6 |
| B2 | 4.3 | D1 | 4.16 → 5.5 |
