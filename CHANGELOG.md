...
* `TransformPublisher.close()` and `flush()` no longer let a wrapped
  publisher's synchronous throw escape to the caller's call site: the
  future they handed back fails instead, as `MultiPublisher` has always
  done. `close()` is terminal even when the wrapped `close()` throws before
  its first `await`. The synchronous
